...
type Habit = {
  ...
  export default function Habits({habits}: HabitState) {
    ...
    const calculatedProgress = (days:number):number => {
      return Math.min((days / 66)*100, 100);
    }

   return (
   <div className="w-full max-w-md p-4 bg-white rounded-lg shadow-md mt-8">
   <h1 className="text-2xl font-bold mb-1 text-black">Habits</h1>
   <ul className="space-y-4">
      ...
      <progress className="w-24" value={calculatedProgress(habit.days)} max="100"></progress>
      <button className="px-2 py-1 text-sm text-white bg-blue-500 rounded" onClick={()=> handleMarkAsDone(habit._id,dispatch)}> {status[habit._id]==="loading"? "Processing":"Mark as done"}</button> 
        ...
        {status[habit._id]==="success" && <span className="text-green-500"> Done! </span>}
   </div>
   </li>
   ))}
   </ul>
